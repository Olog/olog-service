ALTER TABLE `logs` ADD COLUMN `level` ENUM('Info','Problem','Request','Suggestion','Urgent') NOT NULL DEFAULT 'Info' AFTER `state`;
UPDATE `logs` SET `level`='Info' WHERE `level_id`='1';
UPDATE `logs` SET `level`='Problem' WHERE `level_id`='2';
